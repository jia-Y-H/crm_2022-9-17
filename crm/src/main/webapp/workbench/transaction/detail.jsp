...
String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/";
	...
	for(int i=0; i<dvList.size(); i++){
		DicValue dv = dvList.get(i);	//取得每一个字典值
		//从dv中取得value
		String stage = dv.getValue();
		//根据stage取得possibility
		String possibility = pMap.get(stage);
		if ("0".equals(possibility)){		//如果可能性为零说明找到了前面正常阶段和后面丢失阶段的分界点
			point = i;
			break;
		}
	}
...
<script type="text/javascript">
	...
	//改变交易阶段  stage:需要改变的阶段  i:需要改变的阶段对应的下标
	...
	function changeIcon(stage, index1){
		...
		if(currentPossibility == "0"){
			...
			for (let i = point; i < <%=dvList.size()%>; i++) {		//遍历后两个
				if(i == index){			//如果是当前阶段
					//红叉-----------------
					$("#"+i).removeClass();
		...
		}else {			//如果当前阶段的可能性不为0,前七个绿圈，绿色标记，黑圈，后两个一定是黑叉
			for (let i = 0; i < point; i++) {		//遍历前七个
				if(i == index){
					//绿色标记------------
					$("#"+i).removeClass();
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//判断当前阶段
			if("0".equals(currentPossibility)){			//如果当前阶段的可能性为0
				for (int i = 0; i < dvList.size(); i++) {
					...
					if ("0".equals(listPossibility)) {		//如果遍历出来的阶段的可能性为0
						if(listStage.equals(currentStage)){		//如果是当前阶段
							//红叉--------------------
		%>
		...
		<%
				...
				for (int i = 0; i < dvList.size(); i++) {
					...
					String listStage = dv.getValue();
					String listPossibility = pMap.get(listStage);
					if ("0".equals(listPossibility)) {		//如果遍历出来的阶段的可能性为0,说明是后两个阶段
						//黑叉--------------------
		%>
						<span id="<%=i%>" onclick="changeStage('<%=listStage%>', '<%=i%>')"
							  class="glyphicon glyphicon-remove mystage"
